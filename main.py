...
# 최초 경로 탐색
actions = bfs(map_data, start, target, WALL_SYMBOL, WATER_SYMBOL, TREE_SYMBOL)
...
while game_data is not None:
    ...
    if not actions:
        start, target = find_positions(map_data, START_SYMBOL, TARGET_SYMBOL)
        actions = bfs(map_data, start, target, WALL_SYMBOL, WATER_SYMBOL, TREE_SYMBOL) if start and target else []
